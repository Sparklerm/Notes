...
<configuration>
    ...
    <!-- 别名：
        1. typeAlias：给指定的实体指定别名
        2. package：给指定包的实体指定别名，默认为类名小写，可用@Alias("")注解指定别名
     -->
    <typeAliases>
    ...
    -->
    <typeHandlers>
        <typeHandler handler="java.lang.Integer" javaType="integera"/>
    </typeHandlers>
    ...
    <mappers>
        ...
        <!-- 使用映射器接口实现类的完全限定类名， SQL映射配置文件需要和Mapper文件处于同一文件夹且同名-->
