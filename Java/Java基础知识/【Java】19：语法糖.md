...
看到这个代码，你知道原来**字符串的switch是通过****equals()****和****hashCode()****方法来实现的。**还好hashCode()方法返回的是int，而不是long。
...
通过反编译后代码我们可以看到，public final class T extends Enum，说明，该类是继承了Enum类的，同时final关键字告诉我们，这个类也是不能被继承的。**当我们使用****enmu****来定义一个枚举类型的时候，编译器会自动帮我们创建一个****final****类型的类继承****Enum****类，所以枚举类型不能被继承。**
...
### [可能遇到的坑](http://hollischuang.gitee.io/tobetopjavaer/#/basics/java-basic/syntactic-sugar?id=可能遇到的坑)
...
### [总结](http://hollischuang.gitee.io/tobetopjavaer/#/basics/java-basic/syntactic-sugar?id=总结)
...
参考资料： [Java的反编译](http://www.hollischuang.com/archives/58) [Java中的Switch对整型、字符型、字符串型的具体实现细节](http://www.hollischuang.com/archives/61) [深度分析Java的枚举类型—-枚举的线程安全性及序列化问题](http://www.hollischuang.com/archives/197) [Java的枚举类型用法介绍](http://www.hollischuang.com/archives/195) [Java中的增强for循环（for each）的实现原理与坑](http://www.hollischuang.com/archives/1776) [Java中泛型的理解](http://www.hollischuang.com/archives/230) [Java中整型的缓存机制](http://www.hollischuang.com/archives/1174) [Java中的可变参数](http://www.hollischuang.com/archives/1271)